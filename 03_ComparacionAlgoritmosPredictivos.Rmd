...
\end{flushleft}
  ...
  Las variables temporales (estación, mes, día del mes, día de la semana y hora de la lectura del contaminante) fueron incluidas en su forma categórica original y luego de ser codificadas de manera cíclica a través de la inclusión de sus senos y cosenos. Es importante destacar que todas las variables numéricas se sometieron a un proceso de estandarización o normalización. Esto implicó restar la media y dividir por la desviación estándar de cada variable, con el propósito de uniformar sus escalas y establecer una desviación estándar de 1. Esta metodología se implementó para evitar problemas de escala y facilitar la comparación entre las variables en el análisis.

  La inclusión de variables categóricas en la base de datos para desarrollar el modelo predictivo se llevó a cabo mediante el empleo de la técnica conocida de *One-Hot Encoding* [@draper1998applied]. Este proceso implica la transformación de las variables categóricas en una representación binaria, donde cada categoría se convierte en una nueva variable binaria independiente. La razón detrás del uso de *One-Hot Encoding* radica en la necesidad de representar las variables categóricas de manera que puedan ser incorporadas efectivamente en modelos matemáticos y algoritmos de aprendizaje automático. 
...
\begin{equation*}
\text{Pérdida} = \text{Error}(Y - \widehat{Y}) + \lambda \sum_{i=1}^{n} |w_i| \tag{2}
\end{equation*}
\begin{flushleft}
\setstretch{1}
{\footnotesize
\textbf{Ecuación 2:}
  Función de costo regularizada que implementa el algoritmo de LASSO que introduce un término de penalización L1.
  
\textbf{Componentes}:
\begin{itemize}
    \item \textbf{Y:} valores reales u observados en un conjunto de datos. Estos son los valores de la variable de respuesta que se intentan predecir en un problema de regresión.
    \item \textbf{$\widehat{Y}$:} predicciones del modelo. Representa los valores estimados por el modelo para la variable de respuesta.
    \item \textbf{$\lambda$:} hiperparámetro de ajuste que controla la fuerza de la regularización L1 en el método Lasso. Un valor más alto de $\lambda$ aumenta la penalización de los coeficientes de las características, lo que puede llevar a la selección automática de características al forzar algunos coeficientes a cero.
    \item \textbf{w$_{i}$:} coeficientes asociados con cada una de las características en un modelo de regresión. Estos coeficientes representan la contribución de cada característica a la predicción de la variable de respuesta. En el contexto de Lasso, se aplicará la penalización L1 a estos coeficientes para promover la selección automática de características.
\end{itemize}
}
\end{flushleft}
  ...
  Las variables incluidas en cada una de las bases de datos original y reducidas por selección de características, para cada uno de los usos de suelo se encuentran descriptas en el Anexo 2.
...
\textbf{Componentes}:
\begin{itemize}
    \item \textbf{y$_{i}$:} valor real u observado en el i-ésimo punto de datos en un conjunto de datos. Representa el resultado o la variable de respuesta que se está tratando de predecir o modelar en el contexto de un problema de regresión.
    \item \textbf{$\hat{y_i}$:} predicción del modelo en el i-ésimo punto de datos. Es el valor estimado por el modelo para y$_{i}$.
    \item \textbf{n:} número total de puntos en el conjunto de datos.
\end{itemize}
}
\end{flushleft}

  Error absoluto medio (*mean absolute error ($MAE$)*): mide la media de las diferencias absolutas entre los valores predichos por el modelo y los valores observados, siendo menos sensible a valores atípicos a diferencia del $RMSE$ (Ecuación 4). Una ventaja del $MAE$ es que es fácil de interpretar, ya que se mide en las mismas unidades que la variable de respuesta. Un valor de $MAE$ más bajo indica que el modelo es más preciso. 
...
{\footnotesize
\textbf{Ecuación 4:}
  Error absoluto medio (\textit{mean absolute error} ($MAE$)).
...
{\footnotesize
\textbf{Ecuación 5:}
  coeficiente de determinación (\textit{coefficient of determination} ($R^2$)).
  
\textbf{Componentes}:
\begin{itemize}
    \item \textbf{SSR (suma de cuadrados de la regresión o \textit{Sum of Squares due to Regression}):} representa la variación explicada por el modelo de regresión. Mide cuánto de la variación en la variable dependiente puede ser explicada por las variables independientes o predictoras del modelo. En otras palabras, la SSR representa la contribución del modelo a la explicación de la variación en los datos. 
    \item \textbf{SST (suma de cuadrados totales o \textit{Total Sum of Squares)}:} representa la variación total en los valores observados de la variable dependiente.
\end{itemize}
}
\end{flushleft}

  Donde SSR es la suma de cuadrados de la regresión también conocida como variación explicada por el modelo (Ecuación 6) y SST es la variación total en los datos también conocida como suma de cuadrados totales (Ecuación 7).
...
{\footnotesize
\textbf{Ecuación 6:}
  Suma de cuadrados de la regresión o \textit{Sum of Squares due to Regression}).
  
\textbf{Componentes}:
\begin{itemize}
    \item \textbf{$\hat{y_i}$:} predicción del modelo para el i-ésimo punto de datos.
    \item \textbf{$\overline{y}$:} es la media de los valores observados y$_{i}$.
\end{itemize}
}
\end{flushleft}
...
{\footnotesize
\textbf{Ecuación 7:}
  Suma de cuadrados totales o \textit{Total Sum of Squares}.
  
\textbf{Componentes}:
\begin{itemize}
    \item \textbf{y$_{i}$:} valores observados.
    \item \textbf{$\overline{y}$:} es la media de los valores observados y$_{i}$.
\end{itemize}
}
\end{flushleft}
...
{\footnotesize
\textbf{Ecuación 9:}
  Coeficiente de concordancia de Lin (\textit{Lin's Concordance Correlation Coefficient}).
  
\textbf{Componentes}:
\begin{itemize}
    \item \textbf{s$_{12}$:} covarianza entre las dos mediciones o poblaciones. Mide cómo las dos mediciones varían juntas. Una covarianza positiva indica una relación positiva, mientras que una covarianza negativa indica una relación negativa.
    \item \textbf{$\overline{x}$:} media de las dos mediciones o poblaciones. Estas medias representan el promedio de las mediciones en cada conjunto de datos.
    \item \textbf{s$^2$$_{i}$:} varianza de las dos mediciones o poblaciones. Estas varianzas miden la dispersión o variabilidad de las mediciones en cada conjunto de datos.
\end{itemize}
}
\end{flushleft}
...
#### Base de datos original
  ...
  En esta sección se desarrolló el modelado predictivo a partir de la base de datos que involucró todas las variables obtenidas a partir de información satelital, una descripción detallada de cada una de ellas puede hallarse en el Anexo 1. La base de datos original empleada para llevar a cabo el modelado predictivo involucró un total de 193 variables predictoras. Los resultados del modelo predictivo desarrollado en esta sección se presentan en las tablas \@ref(tab:modurbtrain) y \@ref(tab:modurbtest).
...
\setstretch{1}
{\footnotesize 
Raíz del error cuadrático medio ($RMSE$), error absoluto medio ($MAE$), coeficiente de determinación ($R^2$), coeficiente de correlación de Pearson ($r$) y coeficiente de concordancia de Lin ($CCC$).}
\end{flushleft}
...
#### Base de datos reducida por el algoritmo LASSO
  ...
  En esta sección se desarrolló el modelado predictivo a partir de la base de datos reducida por medio del algoritmo de selección de variables de *LASSO*. La base de datos original quedó reducida a un total de 55 variables predictoras. Los resultados del modelo predictivo desarrollado en esta sección se presentan en las tablas \@ref(tab:modurbtrainLASSO) y \@ref(tab:modurbtestLASSO).
...
#### Base de datos reducida por el algoritmo featurewiz
  ...
  En esta sección se desarrolló el modelado predictivo a partir de una base de datos reducida por medio del algoritmo de selección de variables de *featurewiz*. La base de datos original quedó reducida a un total de 78 variables. Los resultados del modelo predictivo desarrollado en esta sección se presentan en las tablas \@ref(tab:modurbtrainfeat) y \@ref(tab:modurbtestfeat).
...
#### Base de datos original
  ...
  En esta sección se desarrolló el modelado predictivo a partir de la base de datos original que cuenta con un total de 193 variables predictoras, las cuales se encuentran descriptas en el Anexo 1. Los resultados del modelo predictivo desarrollado en esta sección se presentan en las tablas \@ref(tab:modaatrain) y \@ref(tab:modaatest).
...
#### Base de datos reducida por el algortimo de LASSO
  ...
  En esta sección se desarrolló el modelado predictivo a partir de la base de datos reducida por medio del algoritmo de selección de variables de *LASSO*. La base de datos original quedó reducida a un total de 59 variables predictoras. Los resultados del modelo predictivo desarrollado en esta sección se presentan en las tablas \@ref(tab:modaatrainLASSO) y \@ref(tab:modaatestLASSO).
...
#### Base de datos reducida por el algoritmo de featurewiz
  ...
  En esta sección se desarrolló el modelado predictivo a partir de una base de datos reducida por medio del algoritmo de selección de variables de *featurewiz*. La base de datos original quedó reducida a un total de 84 variables. Los resultados del modelo predictivo desarrollado en esta sección se presentan en las tablas \@ref(tab:modaatrainfeat) y \@ref(tab:modaatestfeat).
...
## Discusión
  ...
  Los algoritmos de redes neuronales ajustados no identificaron variables predictoras de gran importancia relativa, a diferencia de lo que sucede con los bosques aleatorios en el uso del suelo agrícola y área verde. En las redes neuronales del uso del suelo urbano, todas las variables aportaron de forma homogénea a las predicciones, siendo el algoritmo predictivo el encargado de internalizar las ponderaciones de las mismas. En las figuras \@ref(fig:impurb), \@ref(fig:impurbLASSO) y \@ref(fig:impurbfeat), se presentaron los porcentaje de importancia relativa para el uso del suelo urbano, los cuales evidenciaron ser inferiores a los del uso del suelo agrícola y área verde. Resultados similares fueron reportados por Ghassen et. al., quienes demostraron que mejores resultados eran alcanzados mientras un mayor número de variables era involucrado al implementar redes neuronales recurrentes, particularmente LSTM [@chniti2017commerce]. 

  Dadas las características de la serie temporal urbana el modelado predictivo fue más apto a través de redes neuronales recurrentes, cuyo algoritmo posee numerosos parámetros que logran reflejar el aporte de cada una de las variables a la predicción bajo las distintas situaciones posibles presentes en la base de datos. Al calcularse una gran cantidad de parámetros, puede notarse reflejado en cada predicción la influencia de cada una de las variables presentes en la base de datos, siendo el algoritmo predictivo el encargado de ponderar las influencias en cada caso particular. Al tratarse de una serie en donde la importancia relativa atribuida a las distintas predictoras es similar, el contar con un algoritmo predictivo que tenga en cuenta la influencia conjunta de todas las variables resultó en un mayor desempeño predictivo. De esta forma en cada predicción puede verse reflejada la influencia de cada una de las variables predictoras. Además, las redes neuronales se desempeñaron mejor cuando se trabajó con la base de datos original en comparación con las reducidas; resultado que posiblemente podría encontrarse asociado con la homogeneidad de las influencias relativas de las variables y las características de aprendizaje del algoritmo de redes neuronales recurrentes. 
 
  En relación con las reducciones de las bases de datos mediante la aplicación de algoritmos de selección de características, observamos que se han mantenido las características más relevantes. Esto se debe a que, a pesar de la notable reducción en el número de variables involucradas, los resultados de los modelos predictivos se han mantenido consistentes tanto en la base de datos de entrenamiento como en la de testeo, independientemente de las diferentes reducciones realizadas. En el caso del uso de suelo urbano, logramos el mejor rendimiento predictivo al emplear redes neuronales recurrentes con la base de datos completa (RMSE de entrenamiento = 1,657 $\frac{\mu g}{m^3}$, RMSE de prueba = 3,747 $\frac{\mu g}{m^3}$). Por otro lado, incluso con menos del 50% de datos de la base original, las reducciones a través de los métodos featurewiz y LASSO obtuvieron resultados de RMSE cercanos. El método featurewiz dio como resultado un RMSE de entrenamiento de 2,091 $\frac{\mu g}{m^3}$ y un RMSE de prueba de 3,804 $\frac{\mu g}{m^3}$, mientras que el método LASSO arrojó un RMSE de entrenamiento de 1,943 $\frac{\mu g}{m^3}$ y un RMSE de prueba de 3,897 $\frac{\mu g}{m^3}$. En cuanto al uso de suelo agrícola y área verde, el mejor rendimiento predictivo se logró mediante el algoritmo de bosques aleatorios con la base de datos reducida mediante el algoritmo LASSO, con un RMSE de entrenamiento de 0,975 $\frac{\mu g}{m^3}$ y un RMSE de prueba de 2,550 $\frac{\mu g}{m^3}$, manteniendo tan solo el 30% de la base de datos completa. Sin embargo, en las demás alternativas de bases de datos, el algoritmo de bosques aleatorios también demostró un buen rendimiento, a pesar de ser ligeramente inferior al de la base de datos reducida por LASSO. Para el método featurewiz, se obtuvo un RMSE de entrenamiento de 1,020 $\frac{\mu g}{m^3}$ y un RMSE de prueba de 2,702 $\frac{\mu g}{m^3}$, mientras que con la base de datos completa, el RMSE de entrenamiento fue de 1,016 $\frac{\mu g}{m^3}$ y el RMSE de prueba de 2,673 $\frac{\mu g}{m^3}$.
